  ...
  useEffect(() => {
    ...
    const fetchMonthly = async () => {
      try {
        ...
        const url = `/admin/musics/${music.id}/rewards/monthly?` + params.toString()
  ...
  useEffect(() => {
    ...
    const fetchTrend = async (kind: 'music' | 'lyrics', granularity: 'daily' | 'monthly') => {
      ...
      try {
        ...
        const url = `/admin/musics/${music.id}/rewards/trend?` + params.toString()
        const res = await fetch(url)
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        const data = await res.json()
        if (aborted) return
  ...
  useEffect(() => {
    ...
    const fetchCompanies = async () => {
      try {
        ...
        const url = `/admin/musics/${music.id}/rewards/companies?` + params.toString()
        const res = await fetch(url)
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        const data = await res.json()
        if (aborted) return
